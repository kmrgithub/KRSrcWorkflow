...
namespace StructuredStorageFileParser
{
	...
	public class StructuredStorageFileParser : Processor<StructuredStorageFileParserData>
	{
		...
		public override void Process(StructuredStorageFileParserData data)
		{
			...
			ParseFile(data.DocumentToProcess, this.Depth, ref outputfiles);
			foreach (string outputfile in outputfiles)
			{
				string filenm = outputfile;
				KRSrcWorkflow.WFFileType.FileType type = KRSrcWorkflow.WFFileType.GetFileType(outputfile);
				if (type != KRSrcWorkflow.WFFileType.FileType.Unknown)
				{
					string newfilenm = filenm;
					string extension = KRSrcWorkflow.WFFileType.GetFileTypeExtension(type);
				...
				data.OutputDocuments.Add(filenm);
			}
			data.WFState.Value = "Process";
		}
	}
}
